...
async def upsert_children(client_id: str, children: List[ChildPayload], user_auth = Depends(verify_token)):
    ...
    try:
        ...
        # normalize possible error structure
        def extract_error(resp):
            if resp is None:
                return None
            # ApiResponse object may expose attributes or be a dict
            err = None
            try:
        ...
        del_err = extract_error(del_res)
        if del_err:
            # Log but continue; deletion failure is non-fatal in this flow
            print('Warning: failed to delete existing children for', client_id, del_err)
        ...
        if children and len(children) > 0:
            ...
            ins_err = extract_error(ins_res)
            if ins_err:
                raise Exception(ins_err)
